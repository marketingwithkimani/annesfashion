...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Pre-order & Flash sale toggles
    const preOrderToggle = document.getElementById('globalPreOrderToggle');
    if (preOrderToggle) preOrderToggle.addEventListener('change', handleGlobalPreOrderToggle);
    const flashSaleToggle = document.getElementById('globalFlashSaleToggle');
    if (flashSaleToggle) flashSaleToggle.addEventListener('change', handleGlobalFlashSaleToggle);

    // Shipment tracking broadcast form
    const shipmentForm = document.getElementById('shipmentUpdateForm');
    if (shipmentForm) shipmentForm.addEventListener('submit', handleShipmentUpdateSubmit);
...
async function loadGlobalSettings() {
    try {
        // Load pre_order_mode
        const poRes = await fetch(`${API_BASE}/settings/get.php?key=pre_order_mode`, {
            headers: { 'Authorization': `Bearer ${authToken}` }
        });
        const poData = await poRes.json();
        if (poData.success) {
            const toggle = document.getElementById('globalPreOrderToggle');
            if (toggle) toggle.checked = poData.data.value === 'on';
        }

        // Load flash_sale_mode
        const fsRes = await fetch(`${API_BASE}/settings/get.php?key=flash_sale_mode`, {
            headers: { 'Authorization': `Bearer ${authToken}` }
        });
        const fsData = await fsRes.json();
        if (fsData.success) {
            const toggle = document.getElementById('globalFlashSaleToggle');
            if (toggle) toggle.checked = fsData.data.value === 'on';
        }

        // Load recent shipment broadcast updates
        loadRecentShipmentUpdates();
    } catch (error) {
...
async function handleGlobalFlashSaleToggle(e) {
    const isOn = e.target.checked;
    try {
        const response = await fetch(`${API_BASE}/settings/update.php`, {
            method: 'POST',
        ...
        const data = await response.json();
        if (data.success) {
            showToast(`Flash Sale Mode turned ${isOn ? 'ON' : 'OFF'}`);
        } else {
            showToast('Failed to update setting');
            e.target.checked = !isOn;
        }
...
async function handleShipmentUpdateSubmit(e) {
    e.preventDefault();
    const title = document.getElementById('shipmentUpdateTitle').value.trim();
    const statusBadge = document.getElementById('shipmentUpdateBadge').value;
    const waybill = document.getElementById('shipmentUpdateWaybill').value.trim();
    const updateText = document.getElementById('shipmentUpdateText').value.trim();

    if (!title || !updateText) {
        showToast('Please fill in title and update text');
        return;
    }

    const btn = document.getElementById('btnPostShipmentUpdate');
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Broadcasting...';

    try {
        // Post directly to Cloudflare Worker API (or API_BASE)
        const workerApiUrl = 'https://client-api.marketingwithkimani.workers.dev';
        const res = await fetch(`${workerApiUrl}/api/tracking/updates`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                title,
        ...
        if (data.success) {
            showToast('Live shipment update broadcasted to customers!');
            document.getElementById('shipmentUpdateForm').reset();
            loadRecentShipmentUpdates();
        } else {
            showToast(data.error || 'Failed to post update');
        }
    } catch (err) {
        console.error('Error posting shipment update:', err);
        showToast('Error posting shipment update');
    } finally {
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-paper-plane"></i> Broadcast Live Status Update';
    }
}

async function loadRecentShipmentUpdates() {
    const list = document.getElementById('recentShipmentUpdatesList');
    if (!list) return;
    ...
    try {
        ...
        if (data.success && data.updates && data.updates.length > 0) {
            list.innerHTML = data.updates.map(u => `
                <div style="background: rgba(0,0,0,0.25); border: 1px solid #333; border-radius: 6px; padding: 10px 12px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                        <strong style="color: #fff; font-size: 0.85rem;">${escapeHtml(u.title)}</strong>
                        <span style="background: var(--primary-gold, #c9a96e); color: #000; font-size: 0.7rem; font-weight: 700; padding: 2px 6px; border-radius: 4px;">
                            ${escapeHtml(u.status_badge || 'In Transit')}
                        </span>
                    </div>
                    <p style="color: #ccc; font-size: 0.8rem; margin: 0 0 4px 0;">${escapeHtml(u.update_text)}</p>
                    <div style="display: flex; justify-content: space-between; font-size: 0.72rem; color: #888;">
                        <span>${u.waybill_number ? `Waybill: ${escapeHtml(u.waybill_number)}` : 'Batch update'}</span>
                        <span>${u.created_at || ''}</span>
                    </div>
                </div>
            `).join('');
        } else {
            list.innerHTML = '<p style="color: #777; font-size: 0.8rem; margin: 0;">No status updates posted yet.</p>';
        }
    } catch (err) {
        list.innerHTML = '<p style="color: #777; font-size: 0.8rem; margin: 0;">Failed to load recent updates.</p>';
    }
}
